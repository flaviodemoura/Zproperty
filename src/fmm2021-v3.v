...
Definition Z_comp {A:Type} (R R1 R2: Rel A) := (forall x y, R x y <-> (R1 !_! R2) x y) /\ exists (f1 f2: A -> A), f_is_Z R1 f1 /\ (forall a b, R1 a b -> (refltrans R) ((f2 # f1) a) ((f2 # f1) b)) /\ (forall a b, b = f1 a -> (refltrans R) b (f2 b)) /\ (f_is_weak_Z R2 R (f2 # f1)).
...
Definition Z_comp_eq {A:Type} (R R1 R2: Rel A) := (forall x y, R x y <-> (R1 !_! R2) x y) /\ exists (f1 f2: A -> A), (forall a b, R1 a b -> (f1 a) = (f1 b)) /\ (forall a, (refltrans R1) a (f1 a)) /\ (forall b a, a = f1 b -> (refltrans R) a (f2 a)) /\ (f_is_weak_Z R2 R (f2 # f1)).

Corollary Z_comp_eq_corol {A:Type}: forall (R R1 R2: Rel A), Z_comp_eq R R1 R2 -> Z_comp R R1 R2.
Proof.
  intros R R1 R2 HZ_comp_eq.
  unfold Z_comp.
  unfold Z_comp_eq in HZ_comp_eq.
  destruct HZ_comp_eq as [Hunion HZ_comp_eq].
  split.
  - assumption.
  - destruct HZ_comp_eq as [f1 [f2 [H1 [H2 [H3 H4]]]]].
    exists f1, f2.
    split.
    + unfold f_is_Z.
      intros a b Hab.
...
Lemma Z_comp_eq_implies_Z_prop {A:Type}: forall (R R1 R2: Rel A), Z_comp_eq R R1 R2 -> Z_prop R.
Proof.
  intros R R1 R2 HZ_comp_eq.
