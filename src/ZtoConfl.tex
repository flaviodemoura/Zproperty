\section{The Z property implies Confluence}
  ...
  the reflexive transitive closure of a relation \coqdocvar{R}, written as
...
axiom for $\tto_R$, while \coqdocvar{rtrans} extends the reflexive transitive
closure of \coqdocvar{R}, if one has at least a one-step reduction. As a first
...
giving the general idea of what they do. The uncommented proof of the
 lemma \coqdocvar{refltrans\_composition} is as follows:
...
\coqdocindent{0.50em}
\coqdoctac{intros} \coqdocvar{t} \coqdocvar{u} \coqdocvar{v}.\coqdoceol
\coqdocindent{0.50em}
...
+ \coqdoctac{assumption}.\coqdoceol
\coqdocindent{1.50em}
+ \coqdoctac{apply} \coqdocvar{IHrefltrans}; \coqdoctac{assumption}.\coqdoceol
\coqdocnoindent
\coqdockw{Qed}.
...
the reserved words \coqdockw{Proof} and \coqdockw{Qed}, each proof command finishes with
a dot, and proofs can be structured with bullets. We now present the 
commented proof of the lemma \coqdocvar{refltrans\_composition}, by writing the 
...
Let \coqdocvar{t},\coqdocvar{u} and \coqdocvar{v} be elements of type \coqdocvar{A}
      (or be elements of the set \coqdocvar{A}).\begin{coqdoccode}
\coqdocemptyline
\coqdocindent{1.00em}
\coqdoctac{intros} \coqdocvar{H1} \coqdocvar{H2}. \end{coqdoccode}
Assume that $t \tto_R u$
      (name this assumption \coqdocvar{H1}) and $u\tto_R v$ (name this assumption
      \coqdocvar{H2}). \begin{coqdoccode}
\coqdocemptyline
\coqdocindent{1.00em}
\coqdoctac{induction} \coqdocvar{H1}. \end{coqdoccode}
The proof proceeds by induction on the hypothesis
      \coqdocvar{H1}, i.e. by induction on $t \tto_R u$. The structure of the
      ...
      \coqdocvar{H1} and \coqdocvar{H2} are the only hypothesis (the other lines are just
      declaration of variables), therefore the induction hypothesis
      subsumes \coqdocvar{H2}.
...
The first case is when $t \tto_R u$ is generated
      by the constructor \coqdocvar{refl}, which is an axiom and hence we are
...
The second case, i.e. the recursive case
      ...
      \coqdocvar{rtrans}. This means that there exists an element, say $b$, such
      that $t \to_R b$ and $b \tto_R u$. Therefore, in order to prove
      that $t \tto_R u$, we can apply the rule \coqdocvar{rtrans} taking \coqdocvar{b} as
...
In the second subgoal, we
    ...
    hypothesis \coqdocvar{IHrefltrans}: $u \tto_R v \to b \tto_R u$, where
    $u\tto_R v$ is the hypothesis \coqdocvar{H2}. \begin{coqdoccode}
\coqdocnoindent
...
$c \tto_R c$ corresponds to an application of
        ...
        \coqdocvar{rtrans}. The induction step in this case will assume that
        ...
        remove the hypothesis \coqdocvar{H}, and replace it by another relevant
...
Let $c_0$ be a reduct of $a$, and \coqdocvar{Hrefl2}
    ...
    Before applying induction to \coqdocvar{Hrefl2}: $a \tto_R c_0$, we will be
    replace the hypothesis \coqdocvar{H}: $a\to_R b$ by two other properties
